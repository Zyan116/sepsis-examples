...
of MDP. The main part of the code is borrowed from data generator:
https://github.com/StanfordAI4HI/off_policy_confounding/blob/master/sepsis/core/generator_confounded_mdp.py
...
class matrix_mdp(object):
    def __init__(self, tx_mat, r_mat, policy, t0_policy, value_fn, config):
        ...
        _select_actions_unconfounded()
            select an action based on behaviour policy -- unconfounded
            returns action index
        _select_actions_confounded()
            select an action based on behaviour policy -- confounded
            returns action index
        select_actions_target()
            select an action based on evaluaiton policy
        """
    ...
    def _select_actions_unconfounded(self):
        """_select_actions_unconfounded
        selects the next action based on the behaviour policy, unconfounded
        Returns
        -------
        next_action : int
    ...
    def _select_actions_confounded(self):
        """_select_actions_confounded
        selects the next action based on the behaviour policy, confounded
        ...
        return next_action, ids
    
    def select_actions_target(self, target_policy):
        """select_actions_target
        selects the next action based on the target policy.
        Parameters
        ----------
        target_policy: np.array, float [n_actions, n_states]
            target policy
        Returns
        -------
        next_action: int
            index of the next action
        """
        next_prob = target_policy[:, self.current_state].squeeze()
...
class data_generator(object):
    def __init__(self, transitions, policies, value_fn, config):
        ...
        """__init__
        wrapper around matrix_mdp to simulate trajectories from the MDP
        ...
        -------
        simulate(num_iters)
            simulates num_iters trajectories and rewards by the MDP
            returns trajectories and discounted returns
        process_data()
            process output of MDP to trajectories and returns
        calc_reward()
        ...
        self.MDP = matrix_mdp(tx_mat, r_mat, policy, t0_policy, 
                                       value_fn, config)
    ...
    def simulate(self, num_iters, use_tqdm=False):
        ...
        iter_ids = np.zeros(num_iters)
        iter_ius = np.zeros(num_iters)

        for itr in tqdm(range(num_iters), disable=not(use_tqdm)):
            # MDP will generate the diabetes index as well
            state, iter_ius[itr] = self.MDP.reset()

            iter_states[itr, 0, 0] = state #self.initial_state
            action, iter_ids[itr] = self.MDP._select_actions_confounded()
            state, reward, terminal = self.MDP.step(action)
            
            iter_actions[itr, 0, 0] = action
            iter_states[itr, 1, 0] = state
            iter_rewards[itr, 0, 0] = reward

            if not terminal :
            
                for step in range(1, self.max_num_steps):
                    action = self.MDP._select_actions_unconfounded()
                    # Take the action
                    state, reward, terminal = self.MDP.step(action)
        ...
        return trajectories, returns, iter_ids, iter_ius
    ...
    def calc_reward(self, trajectories, discount):
        ...
        discounted_reward : float [num_iters]
            discounted return for each trajectory
        """
    ...
    def simulate_pd(self, num_iters, use_tqdm=False):
        """simulates 
        Simulates num_iters trajectories and rewards by MDP
        ...
        trajectories : np.array [num_iters, max_horizon, 5]
            ...
            [num_iters, max_horizon, 3] : next state index
            [num_iters, max_horizon, 4] : reward
        returns : np.array [num_iters, max_horizon]
            discounted returns by config['discount'] discount factor
        ...
        trajectories, returns = self.process_data_pd(
                                iter_states[..., 0], iter_actions[..., 0], 
                                iter_rewards[..., 0], num_iters)
    ...
    def process_data_pd(self, states, actions, rewards, num_iters):
        """process_data 
        process states, actions and rewards into two arrays
        trajectories and rewards for computation of per-decision IS
        ...
        trajectories : np.array [num_iters, max_horizon, 5]
            ...
            [num_iters, max_horizon, 3] : next state index
            [num_iters, max_horizon, 4] : reward
        disc_reward : np.array [num_iters, max_horizon]
            discounted returns by config['discount'] discount factor
        """
        ...
        trajectories[:, :, 3] = states[:, 1:]  # to_states
        trajectories[:, :, 4] = rewards # rewards
        disc_reward = self.calc_reward_pd(trajectories, discount=self.config['discount'])

        return trajectories, disc_reward

    def calc_reward_pd(self, trajectories, discount):
        """calc_reward
        calculates the discounted return for per-decision IS
        Parameters
        ----------
        trajectories : np.array [num_iters, max_horizon, 5]
            output of process_data
        ...
        Returns
        -------
        discounted_reward : np.array [num_iters, max_horzion]
            discounted return for each trajectory
        """
        # Column 0 is a time index, column 4 is the reward
        discounted_reward = (discount**trajectories[..., 0] * trajectories[..., 4])
        return discounted_reward # Take the last axis
    
    def simulate_target(self, num_iters, use_tqdm=False):
        """simulates 
        Simulates num_iters trajectories and rewards by MDP from 
        target policy
        Parameters
        ----------
        num_iters : int
        ...
        trajectories : np.array [num_iters, max_horizon, 5]
            ...
            [num_iters, max_horizon, 3] : next state index
            [num_iters, max_horizon, 4] : reward
        returns : np.array [num_iters, max_horzion]
            discounted returns by config['discount'] discount factor
        ...
        for itr in tqdm(range(num_iters), disable=not(use_tqdm)):
            ...
            for step in range(self.max_num_steps):
                action = self.MDP.select_actions_target()
                # Take the action
                state, reward, terminal = self.MDP.step(action)
